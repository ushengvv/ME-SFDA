...
def train_source(args):
    ...
    for epoch in range(args.max_epoch):
        ...
        if (epoch + 1) % 20 == 0 and epoch +1 !=args.max_epoch:
            torch.save(best_netF, osp.join(args.output_dir, "_"+str(epoch+1)+"_source_F.pt"))
            torch.save(best_netC, osp.join(args.output_dir, "_"+str(epoch+1)+"_source_C.pt"))
            print("_____")
    torch.save(best_netF, osp.join(args.output_dir, "source_F.pt"))
    torch.save(best_netC, osp.join(args.output_dir, "source_C.pt"))


def test_target(args, netF=None, netC=None):
    dset_loaders = office_load(args)
    ## set base network
    if netF==None or netC==None:
        netF = network.ResNet_FE().cuda()
        netC = network.feat_classifier(
            type=args.layer, class_num=args.class_num, bottleneck_dim=args.bottleneck
        ).cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="domainnet_weight")
